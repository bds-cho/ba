resource "aws_instance" "helper-ec2" {
  ...
  user_data = <<-EOL
    ...
    mkdir /root/k6 && cp /ba/k6-workspace/{k6,helper-lambda-s3.js,util.js} /root/k6/ && rm -R /ba
    #cd /root/k6 && ./k6 run --log-format=json --log-output=file=./lambda-s3-<constant/spike>-$(date +%s)-runX.log -e LOAD=<constant/spiky> ./helper-lambda-s3.js
